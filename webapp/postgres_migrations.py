...
def _column(
    ...
    *,
    ...
    return ColumnSpec(name=name, type_name=type_name, nullable=nullable, default_sql=default_sql)
    ...
    TableSpec(
        ...
        (
            ...
            _constraint(
                "c",
                (),
                expression="state in ('staged', 'published', 'superseded', 'failed')",
            ),
            _constraint(
                "c",
                (),
                expression="artifact_storage_mode in ('database', 'object')",
            ),
            _constraint(
                "c",
                (),
                expression="artifact_source in ('version', 'legacy_current')",
            ),
        ),
    ),
    TableSpec(
...
TRIGGER_SPECS = (
    ...
    _trigger(
        ...
        "sqag_generation_evidence",
        ("delete",),
        routine_name="sqag_require_retention_delete_authorization",
    ),
    _trigger(
        "sqag_audit_events_guard_delete",
        "sqag_audit_events",
...
ROUTINE_SPECS = (
    ...
    RoutineSpec(
        ...
        direct_acl=(("sqag_runtime", "EXECUTE", False),),
    ),
)
...
MIGRATION_OBJECTS = (
    ...
    MigrationObjectSpec(
        MIGRATION_FILE_NAMES[1],
        tables=(TABLE_SPECS_BY_NAME["sqag_object_artifacts"],),
    ),
    MigrationObjectSpec(
    ...
    MigrationObjectSpec(
        MIGRATION_FILE_NAMES[6],
        routines=(ROUTINE_SPECS[2],),
    ),
)
...
derived_trigger_links: dict[str, set[tuple[str, str]]] = {}
for trigger in TRIGGER_SPECS:
    derived_trigger_links.setdefault(trigger.routine_key[1], set()).add(
        (trigger.name, trigger.table_name)
    )
if {
    routine: frozenset(links)
    for routine, links in derived_trigger_links.items()
} != EXPECTED_TRIGGER_ROUTINE_LINKS:
...
for migration_object in MIGRATION_OBJECTS:
    for mutation in migration_object.table_mutations:
        table = TABLE_SPECS_BY_NAME.get(mutation.table_name)
        if table is None or not set(mutation.added_columns).issubset(
...
MIGRATION_TABLES = MappingProxyType(
    {
        item.migration_id: frozenset(table.name for table in item.tables)
        ...
        item.migration_id: MappingProxyType(
            {
                ...
                "table_mutations": tuple(
                    (mutation.table_name, mutation.added_columns)
...
for migration_index, migration_object in enumerate(MIGRATION_OBJECTS):
    for table in migration_object.tables:
        key = ("table", f"public.{table.name}")
        if key in _OBJECT_PROVENANCE_RANK:
            raise RuntimeError("SQAG table provenance must be unique.")
        _OBJECT_PROVENANCE_RANK[key] = migration_index
    for index in migration_object.indexes:
        key = ("index", f"public.{index.name}")
        if key in _OBJECT_PROVENANCE_RANK:
            raise RuntimeError("SQAG index provenance must be unique.")
        _OBJECT_PROVENANCE_RANK[key] = migration_index
    for trigger in migration_object.triggers:
        key = ("trigger", f"public.{trigger.table_name}.{trigger.name}")
        if key in _OBJECT_PROVENANCE_RANK:
            raise RuntimeError("SQAG trigger provenance must be unique.")
        _OBJECT_PROVENANCE_RANK[key] = migration_index
    for routine in migration_object.routines:
        key = (
            "routine",
            f"public.{routine.name}({routine.identity_arguments})",
        )
        if key in _OBJECT_PROVENANCE_RANK:
    ...
    for mutation in migration_object.table_mutations:
        for column_name in mutation.added_columns:
            key = (
                "column",
                f"public.{mutation.table_name}.{column_name}",
            )
            if key in _OBJECT_PROVENANCE_RANK:
...
def _effective_table_spec(table_name: str, applied_count: int) -> TableSpec:
    """Return the table shape at the end of an applied migration prefix."""
    ...
    return TableSpec(
        name=spec.name,
        columns=tuple(
            column for column in spec.columns if column.name not in pending_columns
        ),
...
def _as_tuple(value: Any) -> tuple[Any, ...]:
    ...
    if isinstance(value, str):
        ...
        if stripped.startswith("{") and stripped.endswith("}"):
            ...
            return tuple(item for item in stripped.split(",") if item)
        if not stripped:
            return ()
    return (value,)
...
def _semantic_sql_tokens(value: str | None, *, expression: bool = False) -> tuple[str, ...]:
    normalized = _SQL_CAST_RE.sub("", str(value or ""))
    normalized = re.sub(
        r"=\s*any\s*\(\s*array\s*\[",
        " in (",
        normalized,
...
def _canonical_routine_body(
    routine: RoutineSpec,
    migrations: Sequence[Migration],
) -> str:
    migration = next(
        (item for item in migrations if item.migration_id == routine.source_migration_id),
        None,
    )
    ...
    try:
        ...
        raise MigrationSafetyError(
            f"migration_source_invalid_utf8:{routine.source_migration_id}"
        ) from exc
    matches = list(
        re.finditer(
...
where n.nspname = 'public' and left(c.relname, 5) = 'sqag_'
...
where table_namespace.nspname = 'public' and c.conindid <> 0
"""
    ).fetchall()
...
"""
    ...
    relation_by_oid: dict[Any, dict[str, Any]] = {}
    for table in tables.values():
        relation_by_oid[table["relation"]["oid"]] = table
    for row in rows:
...
"""
    ...
    for row in constraint_rows:
        ...
        if table is None:
            continue
        kind = str(_row_value(row, "contype"))
        columns = tuple(str(item) for item in _as_tuple(_row_value(row, "columns")))
        referenced_columns = tuple(
            str(item) for item in _as_tuple(_row_value(row, "referenced_columns"))
        )
        table["constraints"].append(
            (
                ...
                _semantic_sql_tokens(
                    str(_row_value(row, "expression") or ""),
                    expression=True,
                )
                if kind == "c"
                else (),
...
select index_class.oid as index_oid, index_class.relname as index_name,
       ...
       array(
         select pg_get_indexdef(index_info.indexrelid, key_no, true)
         from generate_series(1, index_info.indnkeyatts) as key_no
         order by key_no
       ) as key_semantics,
...
"""
    ...
    return {
        str(_row_value(row, "index_name")): {
            ...
            "key_semantics": tuple(
                str(item) for item in _as_tuple(_row_value(row, "key_semantics"))
            ),
...
"""
    ...
    return {
        str(_row_value(row, "trigger_name")): {
            ...
            "columns": tuple(
                str(item) for item in _as_tuple(_row_value(row, "columns"))
            ),
            "routine_key": (
        ...
        grouped.setdefault(key, []).append(
            {
                ...
                "proconfig": tuple(
                    str(item) for item in _as_tuple(_row_value(row, "proconfig"))
                ),
                "function_body": str(_row_value(row, "function_body") or ""),
                "function_definition": str(
                    _row_value(row, "function_definition") or ""
                ),
...
"""
    ...
    for row in acl_rows:
        ...
        if _row_value(row, "grantee") is None:
            continue
        acl_grouped.setdefault(key, []).append(
            {
                "grantee": str(_row_value(row, "grantee")),
...
def _column_fingerprint(spec: ColumnSpec) -> tuple[Any, ...]:
    return (
        ...
        spec.type_name,
        spec.nullable,
        _semantic_sql_tokens(spec.default_sql, expression=True)
        if spec.default_sql is not None
        else None,
        spec.identity,
        spec.generated,
    )
...
def _constraint_fingerprint(spec: ConstraintSpec) -> tuple[Any, ...]:
    return (
        ...
        spec.on_delete,
        spec.on_update,
        _semantic_sql_tokens(spec.expression, expression=True)
...
def _table_matches(table: dict[str, Any], spec: TableSpec) -> bool:
    ...
    if (
        ...
        return False
    expected_columns = tuple(_column_fingerprint(column) for column in spec.columns)
    actual_columns = tuple(
        (
            name,
            type_name,
            nullable,
            _semantic_sql_tokens(default_sql, expression=True)
    ...
    expected_constraints = {
        _constraint_fingerprint(constraint) for constraint in spec.constraints
    }
    return set(table["constraints"]) == expected_constraints
...
def _index_matches(observed: dict[str, Any], spec: IndexSpec) -> bool:
    return (
        ...
        and tuple(
            _semantic_sql_tokens(item, expression=True)
            for item in observed["key_semantics"]
        )
        == tuple(
            _semantic_sql_tokens(item, expression=True)
            for item in spec.key_semantics
        )
        and (
...
def _routine_matches(
    ...
    if (
        ...
        or observed["proconfig"] != spec.proconfig
    ):
        return False
    try:
    ...
    if spec.referenced_relations:
        ...
        if relations != set(spec.referenced_relations) or _UNQUALIFIED_SQL_RELATION_RE.search(
            observed["function_definition"]
        ):
            return False
    if spec.direct_acl:
        ...
        if ("sqag_runtime", "EXECUTE", False) not in actual_acl:
            return False
        if any(item not in expected_acl and item[0] != "sqag_migrator" for item in actual_acl):
            return False
    ...
    *,
    ...
    for table_name in sorted(pending_tables):
        if table_name in relation_by_name:
            spec = TABLE_SPECS_BY_NAME[table_name]
            blockers.append(f"pending_suffix_present:table:{_object_key('table', spec)}")

    for migration_object in pending_specs:
        for mutation in migration_object.table_mutations:
    ...
    for item in applied_specs:
        for spec in item.indexes:
            ...
            if observed is None:
                if spec.name in relation_by_name:
                    blockers.append(
                        f"applied_prefix_drift:index:{_object_key('index', spec)}"
                    )
                else:
                    blockers.append(
                        f"applied_prefix_missing:index:{_object_key('index', spec)}"
                    )
    ...
    for index_name in sorted(pending_indexes):
        if index_name in relation_by_name:
            spec = INDEX_SPECS_BY_NAME[index_name]
            blockers.append(
                f"pending_suffix_present:index:{_object_key('index', spec)}"
            )
    ...
    for item in applied_specs:
        for spec in item.triggers:
            ...
            if observed is None:
                blockers.append(
                    f"applied_prefix_missing:trigger:{_object_key('trigger', spec)}"
                )
            elif not _trigger_matches(observed, spec):
                blockers.append(
                    f"applied_prefix_drift:trigger:{_object_key('trigger', spec)}"
                )

    for trigger_name in sorted(pending_triggers):
        if trigger_name in triggers:
            spec = TRIGGER_SPECS_BY_NAME[trigger_name]
            blockers.append(
                f"pending_suffix_present:trigger:{_object_key('trigger', spec)}"
            )
    ...
    for item in applied_specs:
        for spec in item.routines:
            ...
            if not candidates:
                blockers.append(
                    f"applied_prefix_missing:routine:{_object_key('routine', spec)}"
                )
            elif len(candidates) != 1 or not _routine_matches(
                candidates[0],
                spec,
                routine_acls.get(spec.key, []),
                migrations,
            ):
                blockers.append(
                    f"applied_prefix_drift:routine:{_object_key('routine', spec)}"
                )

    for routine_key in sorted(pending_routines):
        if routine_key in routines:
            spec = ROUTINE_SPECS_BY_KEY[routine_key]
            blockers.append(
                f"pending_suffix_present:routine:{_object_key('routine', spec)}"
            )

    for trigger_name, observed in triggers.items():
        if trigger_name.startswith("sqag_") and trigger_name not in EXPECTED_TRIGGERS:
            blockers.append(
                f"managed_namespace_extra:trigger:public.{observed['table_name']}.{trigger_name}"
            )
    for routine_key in routines:
        if routine_key[1].startswith("sqag_") and routine_key not in EXPECTED_ROUTINE_KEYS:
            blockers.append(
                "managed_namespace_extra:routine:"
                f"{routine_key[0]}.{routine_key[1]}({routine_key[2]})"
            )
    return blockers
...
def _sort_blockers(blockers: Sequence[str]) -> list[str]:
    ...
    category_priority = {
        "managed_namespace_extra": 10,
        "applied_prefix_missing": 20,
        "applied_prefix_drift": 30,
        "pending_suffix_present": 40,
    }

    def provenance_rank(kind: str, key: str) -> int:
        if kind == "column":
            return _OBJECT_PROVENANCE_RANK.get((kind, key), len(MIGRATION_OBJECTS))
        if kind in object_class_priority:
            return _OBJECT_PROVENANCE_RANK.get((kind, key), len(MIGRATION_OBJECTS))
        return len(MIGRATION_OBJECTS)
    ...
    def sort_key(blocker: str) -> tuple[Any, ...]:
        ...
        if category == "checksum_drift":
            migration_index = next(
                (
                    index
                    for index, migration_object in enumerate(MIGRATION_OBJECTS)
                    if len(parts) == 2 and migration_object.migration_id == parts[1]
                ),
                len(MIGRATION_OBJECTS),
            )
            return (0, 40, migration_index, 0, blocker)
        if len(parts) == 3 and category in category_priority:
            kind, key = parts[1], parts[2]
            return (
                1,
                provenance_rank(kind, key),
                object_class_priority.get(kind, 99),
...
def inspect_postgres_migrations(
    ...
    ledger_relation = next(
        (relation for relation in relations if relation["name"] == LEDGER_TABLE),
        None,
    )
    ...
    if ledger_exists:
                    ...
                    for position, row in enumerate(applied_rows):
                        ...
                        if (
                            ...
                            if (
                                ...
                                blockers.append(
                                    f"checksum_drift:{expected.migration_id}"
                                )
                            else:
        ...
        reserved_present = any(
            relation["name"] != LEDGER_TABLE for relation in relations
        ) or any(name.startswith("sqag_") for name in indexes) or any(
            name.startswith("sqag_") for name in triggers
        ) or any(key[1].startswith("sqag_") for key in routines)
        if reserved_present:
    ...
    if applied_count is not None and not blockers:
        blockers.extend(
            _schema_object_blockers(
                ...
                constraint_index_oids=constraint_index_oids,
            )
        )
