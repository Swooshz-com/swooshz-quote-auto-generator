...
class ProductionReadinessStatusTest(unittest.TestCase):
    ...
    def test_failed_live_object_storage_provider_evidence_keeps_production_blocked(self):
        ...
        with mock.patch("webapp.object_storage._optional_s3_sdk_available", return_value=True):
            status = self.readiness_status(
                ...
                object_storage_evidence_status="passed",
                object_artifact_lifecycle_evidence_status="passed",
                live_object_storage_provider_evidence_status="failed",
            )

        production_blocker_ids = {item["id"] for item in status["production_blockers"]}
        self.assertEqual(status["live_object_storage_provider_evidence"]["status"], "failed")
        self.assertFalse(status["live_object_storage_provider_evidence"]["live_provider_evidence_supported"])
        self.assertIn("object_storage_provider_unavailable", production_blocker_ids)
        self.assertFalse(status["production_ready"])
    ...
    def test_postgres_database_live_evidence_drops_only_database_blocker(self):
        ...
        text = json.dumps(status, sort_keys=True)
        production_blocker_ids = {item["id"] for item in status["production_blockers"]}
        self.assertTrue(status["production_database_evidence"]["production_database_evidence_supported"])
        self.assertNotIn("postgres_neon_database_evidence_missing", production_blocker_ids)
        self.assertIn("db_object_backup_restore_live_evidence_missing", production_blocker_ids)
    ...
    def test_live_db_object_backup_restore_evidence_drops_only_backup_restore_live_blocker(self):
        ...
        text = json.dumps(status, sort_keys=True)
        production_blocker_ids = {item["id"] for item in status["production_blockers"]}
        self.assertEqual(status["live_db_object_backup_restore_evidence"]["status"], "passed")
        self.assertTrue(status["live_db_object_backup_restore_evidence"]["live_db_object_backup_restore_evidence_supported"])
    ...
    def test_object_storage_provider_config_status_is_metadata_only(self):
        ...
        self.assertEqual(provider["missing_fields"], ["SQAG_OBJECT_STORAGE_SECRET_ACCESS_KEY"])
