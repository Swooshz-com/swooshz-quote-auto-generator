...
SAQG/KQAG is moving from local UAT toward real Koncept Images internal testing
and sellable production readiness. Product-visible local/demo/sample shortcuts
should not be preserved as normal app behavior.

Core direction:

- Koncept Images Pte Ltd should become the first real workspace.
...
KQAG remains local-UAT capable, but it is not ready for internal alpha or
production. The biggest product-shape blockers are Load Sample product
surfaces, local/bundled profile and pricing fallbacks, local artifact download
paths, broad exception fallback behavior, and docs that still describe local or
demo data as the operator happy path.
...
| Storage and migration scripts | Create/update local and database storage schemas, migrate local runtime data into DB mode, validate readiness. | Important for the platform-scoped transition. Some modes still preserve local fallback compatibility. | Live tooling; internal-alpha blockers remain. |
...
- No Load Sample button in internal alpha or production.
...
| --- | --- | --- | --- | --- |
| UI copy | `webapp/static/index.html:274` | App shell says operators can upload reference files or load the sample fixture. | Remove from product code. | Replace with real workspace/import wording; no sample happy path. |
| UI button | `webapp/static/index.html:277` | Product UI renders `Load Sample`. | Remove from product code. | Delete button and related product selectors. |
| JS default | `webapp/static/app.js:4` | `DEFAULT_SAMPLE_ID` points at the sample fixture. | Remove from product code. | Move any needed ID into test harness only. |
| JS intake copy | `webapp/static/app.js:73` | Intake subtitle mentions loading the demo fixture. | Remove from product code. | Replace with real workspace/private import wording. |
| JS element wiring | `webapp/static/app.js:341` | Product state references `sampleDetailsButton`. | Remove from product code. | Delete DOM lookup and state updates. |
| JS handler | `webapp/static/app.js:6207` to `webapp/static/app.js:6235` | `setSampleDetails()` fetches sample API, applies sample details, profile, pricing, and image files. | Remove from product code. | Replace tests with seeded setup that does not call product sample API. |
| JS control state | `webapp/static/app.js:11378` to `webapp/static/app.js:11379` | Sample button visibility and disabled state are managed like a normal app control. | Remove from product code. | Delete control state branch. |
| JS click binding | `webapp/static/app.js:11696` | Product click listener calls the sample handler. | Remove from product code. | Delete listener. |
| Server constants | `webapp/server.py` sample root constants | Server knows about repo sample fixture root. | Replace with test-only fixture helper. | Move behind tests or a non-product seeded setup utility. |
| Server helper | `webapp/server.py:13052` to `webapp/server.py:13123` | `sample_dir`, `list_samples`, and `load_sample` load fixture files and resolve profile/pricing IDs. | Replace with test-only fixture helper. | Remove app helper or move to test support module. |
| Server routes | `webapp/server.py:13671` to `webapp/server.py:13683` | Product API exposes `/api/samples` and `/api/samples/{id}`. | Remove from product code. | Delete routes in first follow-up once tests are seeded another way. |
| Repo fixture files | `fixtures/samples/kent-group/sample.json`, `fixtures/samples/kent-group/kent-group.pdf` | Bundled sample data and sample image are reachable through product route. | Keep only under tests with no product path, or historical/audit-only. | Move to test fixture location or keep only if no product route can read it. |
| Sensitive fixture scanner | `scripts/scan_sensitive_fixtures.py` allowlist entries | Current scanner permits known sample fixture markers. | Keep only if fixture remains test-only. | Tighten allowlist after product route removal. |
| Unit tests | `tests/test_webapp.py` sample-route, Load Sample DOM, default sample ID, Kent fixture assertions. | Tests currently protect product-visible sample behavior. | Follow-up implementation needed. | Rewrite tests to assert sample UI/API absence and use seeded test helpers. |
| Playwright smoke | `scripts/playwright-smoke.mjs` sample button clicks and sample restore expectations. | Smoke depends on product-visible Load Sample. | Follow-up implementation needed. | Replace with test-only seeded workspace/profile/pricing/session setup. |
| Playwright stress | `scripts/playwright-ai-basis-chat-stress.mjs` sample button click. | Stress setup uses product sample path. | Follow-up implementation needed. | Seed setup without product sample CTA/API. |
| Live AI sample check | `scripts/playwright-live-sample-ai-check.mjs` sample button and sample image wait. | Live smoke depends on sample path. | Outdated doc/tooling path to delete or rewrite. | Replace with private-data import smoke outside Git. |
| Docs | `docs/internal-uat*.md`, `docs/platform-uat-smoke-runbook.md`, `docs/kqag-current-status.md`, and related indexes | Some docs present sample/local/demo flow as normal UAT or smoke path. | Outdated docs to delete/rewrite. | Rewrite so real operators use real workspace/private imports; tests use seeded fixtures only. |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `DatabaseKqagStorage.list_pricing_references` | DB pricing list requested. | Workspace DB pricing rows. | Local saved packs and bundled packs. | Yes, if host has local private packs. | Yes, list/detail can expose non-workspace packs in DB/platform mode. | Pricing reference list and generated quote basis. | Not strict enough. | Must be removed before internal alpha. |
| `DatabaseKqagStorage.pricing_reference_detail` | DB detail lookup misses or falls through. | Workspace DB reference detail. | `pricing_reference_pack_detail()` local/bundled resolver. | Yes. | Yes. | Pricing detail and quote generation. | Not strict enough. | Must be removed before internal alpha. |
| `load_profile_pack` and `ProfilePack.resolve` | Profile ID/default path requested during generation. | Workspace profile row/artifact expectation. | Local profile pack, bundled/default layout template. | Yes, layout/profile assets can be local. | Possible if IDs/defaults are shared outside workspace storage. | Profile defaults, layout rules, XLSX generation. | Not strict enough. | Must be removed before internal alpha. |
| `PricingReferencePack.resolve` | Pricing reference ID requested outside DB rows. | Selected workspace pricing reference. | Local pack then bundled default. | Yes. | Possible in hosted mode. | Pricing catalog used for quote basis and line items. | Not strict enough. | Must be removed before internal alpha in database/platform/deploy mode. |
| `/api/samples`, `/api/samples/{id}` | Product API request. | Real workspace data/imports. | Repo sample fixture. | No private-file read by default, but trains product toward fake data. | Shared across all sessions. | Details, images, profile, pricing setup. | Product-reachable. | Remove completely from product code in first follow-up. |
| `setSampleDetails()` | Product UI button click. | User-uploaded booth/render images and real workspace data. | Repo sample details, files, profile, pricing. | No private-file read by default, but product-visible fake success. | Shared sample behavior. | Quote basis and generation setup. | Product-reachable. | Remove completely from product code in first follow-up. |
...
- Public bundled references only if explicitly allowlisted, non-private, not
  ...
  data in database/platform/deploy/internal-alpha/production mode.
...
| --- | --- | --- | --- |
| Load Sample product path | UI, JS handler, server helpers/routes, fixtures, tests, Playwright scripts. | Production blocker; follow-up-needed. | First follow-up removes product UI/API/JS/docs and rewrites tests to seeded setup. |
...
| Local pricing-reference fallback in DB storage | DB list/detail merging local/bundled packs. | Production blocker. | Remove before internal alpha; add isolation tests. |
...
| AI local draft fallback | Local starter draft on AI unavailable/failure. | Local-UAT-only unless made explicit failure. | Mode-gate or convert to safe error in internal alpha/production. |
...
No delete-now code candidate is safe enough for this audit PR because current
tests and smoke scripts still rely on several sample/local paths. The first
implementation PR should replace that reliance, then delete the product sample
surfaces in one controlled change.

## Local Storage Architecture Audit

| Dependency | Current role | Local UAT | Internal alpha | Production | Required change |
| --- | --- | --- | --- | --- | --- |
| `QUOTE_DATA_ROOT` | Local profiles, quote sessions, runtime app data. | Acceptable. | Blocked unless explicitly isolated and backed up for a temporary exception. | Not acceptable. | Move workspace data to DB/object storage. |
...
Codex Security scan. Before internal alpha, rerun the plugin scan and attach
...
| `docs/production-readiness-audit.md` | Valid PR #84 baseline, but Load Sample direction is stricter now. | Keep, superseded in part by this audit. | Link or fold in this audit before internal alpha review. |
...
- All internal-alpha blockers above.
