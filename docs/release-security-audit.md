...
| Medium, live DB+object backup/restore preflight scaffold added in this PR | The next production gate needed a safe way to report missing isolated restore targets and missing operator decisions without touching live data. | `scripts/verify_live_db_object_backup_restore.py`, `tests/test_live_db_object_backup_restore_verifier.py`, `docs/platform-scoped-storage-mode.md`, `docs/production-readiness-audit.md` | The scaffold checks required env names by presence only, compares active and restore database/object targets without printing values, reports `blocked_isolated_restore_target_missing` and `blocked_backup_restore_decision_missing`, and never runs backup, restore, database writes, object writes, or destructive operations. Even when prerequisites are present, it reports `live_db_object_backup_restore_execution_not_implemented`, keeps `production_ready=false`, and does not claim evidence. | Implement a real operator-run live restore drill only after an isolated restore database, isolated restore object target, backup ownership decision, and restore-window decision exist outside Git. |
...
17. Live DB+object backup/restore preflight: this PR adds a blocked-only metadata scaffold for isolated restore target and operator decision checks; it does not produce evidence or run restore operations.
...
| Command | Result |
| --- | --- |
| `python scripts/verify_live_db_object_backup_restore.py` | Expected nonzero exit 2. Reported `status=blocked`, `live_db_object_backup_restore_evidence_supported=false`, `blocked_isolated_restore_target_missing`, `blocked_backup_restore_decision_missing`, metadata-only privacy booleans, and `production_ready=false`; it did not run backup, restore, database writes, object writes, or destructive operations. |
| `python -m py_compile scripts/verify_live_db_object_backup_restore.py` | Passed. |
| `python -m unittest tests.test_live_db_object_backup_restore_verifier tests.test_database_backup_restore_verifier tests.test_object_artifact_lifecycle_verifier tests.test_production_readiness` | Passed: 43 tests OK, covering blocked live DB+object backup/restore preflight, metadata-only output, isolated restore target checks, no evidence overclaim, synthetic backup/restore, synthetic object lifecycle, and readiness blockers. |
