...
metadata-only by default; it performs a read-only live schema check only when
operators explicitly enable `SQAG_LIVE_DATABASE_EVIDENCE`.
...
| Medium, Postgres metadata adapter boundary added in this PR | The production DB path needed a real Postgres/Neon-compatible runtime boundary instead of only a readiness scaffold. | `requirements.txt`, `webapp/server.py`, `scripts/verify_production_database_provider.py`, `scripts/check_production_readiness.py`, `tests/test_postgres_metadata_storage.py`, `tests/test_production_database_provider_verifier.py`, `tests/test_production_readiness.py`, `tests/test_webapp.py` | Postgres-compatible DB URLs are recognized without printing values; the runtime uses a pinned Psycopg dependency, dialect-aware schema checks, workspace-bound metadata queries, and fail-closed handling for missing drivers, connection failures, missing schema, missing workspace context, unsupported schemes, and Postgres DB-BLOB artifact mode. SQLite remains local-UAT/synthetic only, DB rows store metadata only, and generated XLSX/PDF bytes stay in object storage. | Production still requires operator-run live DB evidence, live DB+object backup/restore, retention/delete, operations, hosted logging/smoke, live Platform smoke, and final audit. |
