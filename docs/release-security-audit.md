...
17. Live DB+object backup/restore drill path: this PR extends the blocked-only scaffold into an opt-in operator drill for synthetic active DB/object writes, restore-target synthetic visibility probes before restore writes, isolated restore DB/object writes, DB+object pairing verification, cleanup, and readiness credit for only `db_object_backup_restore_live_evidence_missing` after a passing non-test-injected run.
