...
| Medium, production DB readiness boundary added in this PR | The SQLite-only production DB blocker did not expose a precise Neon/Postgres-compatible metadata storage gate. | `scripts/verify_production_database_provider.py`, `scripts/check_production_readiness.py`, `webapp/server.py`, `tests/test_production_database_provider_verifier.py`, `tests/test_production_readiness.py`, `tests/test_webapp.py` | Readiness now reports `database_family` and `production_database_evidence`; Postgres-compatible DB URLs are recognized without printing values, SQLite remains local-UAT/synthetic only, metadata migrations for workspace rows and object-artifact metadata are checked by file name only, and runtime Postgres support fails closed instead of falling back to SQLite/local/DB-BLOB storage. | Production still requires a real Postgres/Neon runtime adapter, operator-run live DB evidence, live DB+object backup/restore, retention/delete, operations, hosted logging/smoke, live Platform smoke, and final audit. |
...
- Production-grade Neon/Postgres-compatible database runtime support and live DB evidence. The current checker is metadata-only and fail-closed; it does not connect to or mutate a live database, and it does not make SQLite production storage.
