...
- Production-grade live Neon/Postgres-compatible database evidence. The runtime adapter boundary now exists for metadata rows, but the checker remains metadata-only and fail-closed by default; it connects only under explicit operator live-evidence opt-in, and it does not make SQLite production storage.
