...
S3-compatible provider configuration env names. It stores, retrieves, checks,
...
responses. No live provider evidence is claimed unless an operator runs it with
real provider configuration; test-injected or synthetic backends are never
credited as live provider evidence.
...
| Medium, live-provider evidence path added in this PR | Live S3-compatible object-storage provider proof had no safe metadata-only verifier path. | `scripts/verify_live_object_storage_provider.py`, `scripts/check_production_readiness.py`, `webapp/server.py`, `tests/test_live_object_storage_provider_verifier.py`, `tests/test_production_readiness.py`, `tests/test_webapp.py` | Operators can run an explicit opt-in verifier for synthetic XLSX/PDF store/retrieve/checksum/content-type/byte-size/wrong-workspace/delete/tombstone/missing-object checks; no-env, incomplete-env, and test-injected backends fail closed or remain non-crediting. | Operator-run live provider evidence remains pending unless real provider env is supplied outside Git; production still requires live DB+object backup/restore, live retention/delete, operations, observability, supply-chain hardening, live Platform smoke, and final audit. |
...
15. Live object-storage provider evidence path: this PR adds an opt-in metadata-only verifier and readiness flag; operator-run live provider evidence remains required before production.
...
19. Platform integration audit: source-contract audit added in this PR against current Swooshz Platform main; live Platform-to-KQAG smoke remains required before production.
...
| Command | Result |
| --- | --- |
| `python -m unittest tests.test_live_object_storage_provider_verifier tests.test_object_storage_provider_config tests.test_production_readiness` | Passed: 36 tests OK, covering live-provider verifier no-env/incomplete/test-injected behavior, object provider metadata-only status, and readiness credit boundaries. |
...
| `KQAG_STORAGE_MODE=database KQAG_ARTIFACT_STORAGE_MODE=object KQAG_DATABASE_URL=<synthetic-sqlite-url> python scripts/check_production_readiness.py --with-backup-restore-evidence --with-hosted-observability-evidence --with-hosted-smoke-evidence --with-object-storage-evidence --with-object-artifact-lifecycle-evidence --with-live-object-storage-provider-evidence --backup-restore-work-dir _tmp\validation\live-provider-pr-backup --hosted-observability-work-dir _tmp\validation\live-provider-pr-observability --hosted-smoke-work-dir _tmp\validation\live-provider-pr-smoke --object-storage-work-dir _tmp\validation\live-provider-pr-object-contract --object-artifact-lifecycle-work-dir _tmp\validation\live-provider-pr-lifecycle` | Expected nonzero exit 2. Reported workspace-scoped DB app records, object artifact mode, synthetic backup/observability/smoke/object contract/lifecycle evidence `passed`, live object-provider evidence `failed`, `internal_alpha_ready=false`, `production_ready=false`, and remaining blockers `sqlite_not_final_production`, `object_storage_provider_unavailable`, `object_retention_delete_live_evidence_missing`, `db_object_backup_restore_live_evidence_missing`, `session_business_hardening_incomplete`, and `production_deployment_operations_evidence_missing`. |
...
| `python -m unittest discover -s tests` | Initial sandbox run failed with Windows temp-directory `PermissionError`; escalated rerun passed: 579 tests OK. |
...
- Operator-run live object-storage provider runtime evidence, because no real provider configuration or credentials are supplied in this PR; the new verifier fails closed when env is absent or incomplete.
