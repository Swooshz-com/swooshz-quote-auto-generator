...
- `Nixpacks Python-only contract`: requires `Retrospective fixture integrity and RED reproduction` to have succeeded. It validates that `nixpacks.toml` is parsed completely with Python 3.12 `tomllib` and matches the exact closed build contract: providers exactly `["python"]`, `[phases.setup].nixpkgsArchive` exactly the immutable Nixpkgs archive `5c994fe2b1e540ff83aa59ba370918ad5aae4776` for Python `3.12.13`, and `[start].cmd` exactly `python webapp/server.py`. Invalid TOML syntax, duplicate keys/tables, unknown top-level or nested keys, wrong value types, wrong or additional providers, missing keys, branch/tag/floating archive references, malformed SHAs, wrong start commands, misplaced archive configuration, shadowing/alternate configuration, and any additional content that would expand the locked production build contract fail closed. `.python-version` must bind exactly `3.12.13`, no Node/Dockerfile/Procfile production runtime may be configured, and `requirements.txt` remains the production dependency source. This job is a required successful prerequisite of `Validate app`.
- `Validate app`: requires successful results from `Retrospective fixture integrity and RED reproduction`, `Secret scan`, `Dependency audit`, and `Nixpacks Python-only contract`. Its `always()` condition explicitly requires `needs.nixpacks-contract.result == 'success'`, so a failed, skipped, or cancelled Nixpacks contract prevents the expensive validation workload from starting.
...
- Runtime privilege-contract static validation runs `python scripts/validate_runtime_privilege_contract.py`; the canonical manifest, complete unfiltered protected-role membership evaluation across parent/member/grantor positions, closed runtime-as-member/provider-control schema, exact six-column membership tuple, sixteen-key bounded verification-query set, independent executable-token contracts, exact publication-artifact column authority, the legacy-optional `sqag_quote_artifacts` ordinary-view bounded read, the closed ordinary/materialized-view relation proof, the Boundary B owner-authority model, complete `r`/`S`/`f`/`n`/`T` default-ACL object-class binding, migration-derived `attgenerated`/`attidentity` column semantics, baseline-aware persistent system-routine authority, closed-world `sqag_*` role identity, and repository requirement binding fail closed.
- The runtime contract also correlates external trigger evidence to its target relation and function schema, rejects unexpected standalone unique indexes while distinguishing constraint-backed indexes, requires `pg_constraint.convalidated=true`, rejects PUBLIC default privileges across the covered object classes, preserves casts while normalizing routine bodies, and requires the manifest-prescribed PUBLIC CONNECT ACL in addition to direct runtime CONNECT, binds non-view rewrite-rule posture and foreign-key `confmatchtype` in the migration-derived table model, requires direct `sqag_migrator` CONNECT/CREATE/TEMPORARY provenance from the database owner, checks metadata-only system-relation ACL drift against PostgreSQL `pg_init_privs`, enforces closed-world runtime startup-default semantics with PostgreSQL precedence and explicit policies, preserves executable CHECK casts while normalizing only proven catalog presentation, and requires the retained `sqag_app` rollback-role posture. PostgreSQL regressions cover H39-H58 causal controls, including A19 collation, database-operability, relation-hierarchy identity, A20 generated/identity modes, baseline-aware system-routine deltas, and closed-world SQAG role identity, and preserve H36 as DEFERRED_NONBLOCKING_UNCHANGED.
- Disposable PostgreSQL 17 runtime privilege-contract tests exercise the sixteen canonical query keys, exact result shapes, the automatic creator-admin control edge with ADMIN true, INHERIT false, and SET false, creator REVOKE non-removability, absence of inherited/SET/effective runtime authority, the real publication checksum-backfill path and its prescribed legacy-optional `sqag_quote_artifacts` ordinary-view read (absent on fresh migration databases and conditionally accepted when present), materialized-view runtime SELECT/ownership/grant-option rejection, the exact Boundary B owner authority split for database/schema ACL and PUBLIC TEMPORARY operations, complete table/column privilege matrices, complete default-ACL object classes, closed-world startup-default inventory, semantic policy, precedence, ambiguity, and unknown-setting controls, migration-derived table constraints/indexes/rules and FK match semantics, executable CHECK-cast mutations and harmless catalog formatting, exact retained rollback-role existence and attributes, exact routine bodies and trigger bindings, database/schema/migrator grantor and grant-option semantics, metadata-only system ACL baseline controls, generated/identity column drift, direct/PUBLIC/effective/grant-option system-routine deltas, forbidden/additional/missing SQAG roles, and disposable service databases and roles only. The hosted exact-head evidence must report zero skips.
...
- Gate 3 A19 controls are required and non-optional: clean and mutated PostgreSQL 17 evidence must prove stable column collation identity, exact `datallowconn=true`/`datconnlimit=-1` database posture, both inheritance directions, partition flags, and partition-bound rejection, with exact restoration and zero hosted skips. Run-98 A20 adds the required generated/identity column, baseline-aware system-routine, and closed-world SQAG-role controls with exact restoration and zero hosted skips.
