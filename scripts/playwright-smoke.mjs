...
const run550AuthenticatedOnly = args.includes("--run550-authenticated-only");
const run550AuthenticatedBaseUrl = String(process.env.RUN550_AUTHENTICATED_BASE_URL || "").trim();
let run550PricingDigest = "sha256:2685fa5d3f208d9df578a3dbed4fc2d14fb44c0d2f5d87b9e991a1409719a9b7";
...
const baseUrl = run550AuthenticatedBaseUrl || `http://${options.host}:${options.port}`;
    ...
    await route.fulfill({
      ...
      body: JSON.stringify({
        pricing_reference: {
          ...
          source: run550AuthenticatedOnly ? "company" : "local",
          schema_version: 1,
          currency: "SGD",
          tax: { label: "GST", rate: 0.09 },
          item_count: run550AuthenticatedOnly ? 3 : 1,
          items: [{
    ...
    await route.fulfill({
      ...
      body: JSON.stringify({
        ...
        pricing_references: [{
          ...
          tax: { label: "GST", rate: 0.09 },
          item_count: run550AuthenticatedOnly ? 3 : 1,
          digest_sha256: run550PricingDigest,
        }],
        company_profiles: run550AuthenticatedOnly ? [{
...
async function saveSmokePricingReference(page, internalCost) {
  return page.evaluate(async ({ internalCost, source, authenticatedEvidence }) => postJson("/api/settings/pricing-references", {
    ...
    internalCost,
    source: run550AuthenticatedOnly ? "company" : "local",
    authenticatedEvidence: run550AuthenticatedOnly,
  });
}
...
async function createDashboardSmokeSession(page, suffix, options = {}) {
  return page.evaluate(async ({ suffix, sessionIdPrefix, customerName, projectName, authenticatedEvidence, pricingDigest, dashboardOnly }) => {
    ...
    const generatorProfileId = authenticatedEvidence
      ? "company:synthetic-g3-company-profile"
      : "default";
    ...
    if (authenticatedEvidence) {
      if (!selectPresetValue(companyProfileOptionValue("synthetic-g3-company-profile"))) {
    ...
    state.selectedPresetValue = authenticatedEvidence
      ? companyProfileOptionValue("synthetic-g3-company-profile")
    ...
    if (
      ...
      throw new Error(`Synthetic fixture initial non-generated quote session was not persisted: ${JSON.stringify({
        saved: initialSaved,
    ...
    if (dashboardOnly) return sessionId;
    ...
    authenticatedEvidence: run550AuthenticatedOnly,
    pricingDigest: run550PricingDigest,
    dashboardOnly: options.dashboardOnly === true,
  });
}

async function verifyRun550AuthenticatedExport(page, browser) {
  const session = await page.evaluate(async () => {
    const response = await fetch("/api/session");
    return { status: response.status, body: await response.json() };
  });
  if (session.status !== 200 || !session.body.authenticated) {
    throw new Error("Run-550 native internal-Google login did not create an authenticated session.");
  }

  const savedReference = await saveSmokePricingReference(page, 10);
  if (!savedReference.ok) throw new Error("Run-550 could not save its synthetic pricing reference.");
  run550PricingDigest = String(savedReference.data?.pricing_reference?.digest_sha256 || "");
  if (!/^sha256:[0-9a-f]{64}$/i.test(run550PricingDigest)) {
    throw new Error("Run-550 synthetic pricing reference did not return a canonical digest.");
  }
  const savedProfile = await saveSmokeCompanyProfile(page);
  if (!savedProfile.ok) throw new Error("Run-550 could not save its synthetic company profile.");

  const sessionId = await createDashboardSmokeSession(page, "run550-a", {
    sessionIdPrefix: "quote-run550-a",
    customerName: "Synthetic Run 550 Customer",
    projectName: "Synthetic Run 550 Export",
  });
  const projection = (detail, receipt, targetSessionId) => {
    ...
    return {
      identity: {
        ...
        publicationId: String(xlsx.publication_id || receipt.xlsx?.publication_id || publication.active_publication_id || publication.run_id || ""),
      },
  ...
  const readProjection = async (targetSessionId) => {
    const [detail, receipt] = await Promise.all([
      ...
      page.evaluate(async (id) => {
        const response = await fetch(`/__run550/publication-receipt?session_id=${encodeURIComponent(id)}`);
        return response.ok ? response.json() : {};
      }, targetSessionId),
    ]);
    return projection(detail, receipt, targetSessionId);
  };
  const baseline = await readProjection(sessionId);
  if (
    !baseline.current
    || !baseline.identity.runId
    || !baseline.identity.publicationId
    || !/^[0-9a-f]{64}$/i.test(baseline.digest)
    || !Number.isInteger(baseline.revision)
    || !baseline.url.includes(`/api/quote-sessions/${sessionId}/download/xlsx`)
  ) {
    throw new Error(`Run-550 current publication identity is incomplete: ${JSON.stringify(baseline)}.`);
  }
  const baselineRows = JSON.parse(baseline.arrays.outputRows);
  if (baselineRows.length < 3) throw new Error("Run-550 requires at least three generated rows.");
  if (baselineRows.some((row) => [row.basis_order, row.category_order, row.item_order].includes(1.5))) {
    throw new Error("Run-550 persisted a fractional primary order value.");
  }

  const assertUnchanged = async (label) => {
    const current = await readProjection(sessionId);
    const differingDraftKeys = [...new Set([
      ...Object.keys(baseline.draftState || {}),
      ...Object.keys(current.draftState || {}),
    ])].filter((key) => JSON.stringify(baseline.draftState?.[key]) !== JSON.stringify(current.draftState?.[key]));
    if (
      !current.current
      || current.digest !== baseline.digest
      || current.revision !== baseline.revision
      || current.draftRevision !== baseline.draftRevision
      || JSON.stringify(current.identity) !== JSON.stringify(baseline.identity)
      || JSON.stringify(current.arrays) !== JSON.stringify(baseline.arrays)
    ) {
      throw new Error(`${label} changed canonical persisted commercial state (${differingDraftKeys.join(", ")}): ${JSON.stringify({ ...current, draftState: undefined })}.`);
    }
  };
  ...
  for (const mode of ["pricing_reference", "name", "category", "category_name"]) {
    ...
    await page.evaluate(async (targetSessionId) => {
      const detailed = await loadQuoteSessionDetail(targetSessionId);
      ...
      draft = hydrateDashboardDraftImagePayloads(draft, targetSessionId);
      const restored = await applyQuoteSessionSnapshot(
        { ...draft, quoteSessionId: targetSessionId },
        { sessionId: targetSessionId, forceQuoteView: true, restoreFurthestPanel: true },
      );
      if (!restored) throw new Error("Run-550 reload restore failed.");
      rememberRestoredQuoteSessionBaseline(targetSessionId);
      const saved = await saveQuoteSessionDraftState({ quoteGenerated: true });
      if (!saved) throw new Error("Run-550 reload save failed.");
    }, sessionId);
    await assertUnchanged(`display mode ${mode} reload`);
  }
  if (new Set(Object.values(visibleOrders).map((value) => JSON.stringify(value))).size < 2) {
    throw new Error(`Run-550 display modes did not visibly reorder rows: ${JSON.stringify(visibleOrders)}.`);
  }

  const matching = await page.context().request.get(`${baseUrl}${baseline.url}`);
  if (matching.status() !== 200 || !(await matching.body()).length) {
    throw new Error(`Run-550 matching publication download failed: ${matching.status()}.`);
  }
  const anonymousContext = await browser.newContext();
  try {
    const denied = await anonymousContext.request.get(`${baseUrl}${baseline.url}`);
    if (denied.status() !== 401) {
      throw new Error(`Run-550 unauthenticated download was not rejected: ${denied.status()}.`);
    }
  } finally {
    await anonymousContext.close();
  }

  const secondSessionId = await createDashboardSmokeSession(page, "run550-b", {
    sessionIdPrefix: "quote-run550-b",
    customerName: "Synthetic Run 550 Version Control",
    projectName: "Synthetic Run 550 Version Control",
  });
  const second = await readProjection(secondSessionId);
  if (!second.current || !second.identity.runId || second.identity.runId === baseline.identity.runId) {
    throw new Error("Run-550 could not create a distinct real publication version.");
  }
  const fixtures = await page.evaluate(async ({ sourceSessionId, otherRunId }) => {
    return postJson("/__run550/clone-session", {
      source_session_id: sourceSessionId,
      cross_workspace_session_id: "quote-run550-cross-workspace",
      mismatch_session_id: "quote-run550-wrong-publication",
      mismatch_run_id: otherRunId,
    });
  }, { sourceSessionId: sessionId, otherRunId: second.identity.runId });
  if (!fixtures.ok) throw new Error("Run-550 could not create deterministic publication fixtures.");

  const crossWorkspace = await page.context().request.get(
    `${baseUrl}/api/quote-sessions/quote-run550-cross-workspace/download/xlsx`,
  );
  if (crossWorkspace.status() !== 404) {
    throw new Error(`Run-550 cross-workspace artifact was not rejected: ${crossWorkspace.status()}.`);
  }
  const wrongVersion = await page.context().request.get(
    `${baseUrl}/api/quote-sessions/quote-run550-wrong-publication/download/xlsx`,
  );
  if (wrongVersion.status() !== 404) {
    throw new Error(`Run-550 wrong publication version was not rejected: ${wrongVersion.status()}.`);
  }
  ...
  await page.evaluate(async (targetSessionId) => {
    ...
    if (!restored) throw new Error("Run-550 edit restore failed.");
    setSidePanel("output", { force: true });
  }, sessionId);
  ...
  if (pricedIndex < 0) throw new Error("Run-550 could not locate the 15.00 commercial edit control.");
  ...
  await page.waitForFunction((rowIndex) => (
    Number(state.outputRows[rowIndex]?.unit_price_override) === 16
  ), pricedIndex, { timeout: 15000 });
  await page.evaluate(async () => {
    const saved = await saveQuoteSessionDraftState({ quoteGenerated: true });
    if (!saved) throw new Error("Run-550 real commercial edit did not save.");
  });
  const edited = await readProjection(sessionId);
  if (edited.current || edited.draftRevision === baseline.draftRevision) {
    throw new Error("Run-550 real 15 to 16 commercial edit did not invalidate publication freshness.");
  }
  const stale = await page.context().request.get(`${baseUrl}${baseline.url}`);
  if (stale.status() !== 404) {
    throw new Error(`Run-550 stale publication remained downloadable: ${stale.status()}.`);
  }

  console.log("Run-550 native authenticated export persistence: PASS");
}
...
async function main() {
  let serverInfo = null;
  if (run550AuthenticatedOnly && !run550AuthenticatedBaseUrl) {
    throw new Error("Run-550 authenticated mode requires RUN550_AUTHENTICATED_BASE_URL.");
  }
  ...
  try {
    await installMockProfiles(page);
    if (run550AuthenticatedOnly) {
      await page.goto(`${baseUrl}/login`, { waitUntil: "domcontentloaded", timeout: 15000 });
      if (new URL(page.url()).origin !== new URL(baseUrl).origin) {
        throw new Error("Run-550 synthetic OIDC flow escaped the local harness.");
      }
      await verifyRun550AuthenticatedExport(page, browser);
      console.log(JSON.stringify({
        status: "ok",
        mode: "run550-authenticated",
    ...
    await createDashboardSmokeSession(page, "alpha", { sessionIdPrefix: "quote-7a-playwright-alpha", dashboardOnly: true });
    await createDashboardSmokeSession(page, "beta", { sessionIdPrefix: "quote-2c-hidden-3", dashboardOnly: true });
    await createDashboardSmokeSession(page, "gamma", { sessionIdPrefix: "quote-bulk-extra-1", dashboardOnly: true });
    await createDashboardSmokeSession(page, "delta", { sessionIdPrefix: "quote-bulk-extra-2", dashboardOnly: true });
    await page.reload({ waitUntil: "domcontentloaded" });
    await page.getByRole("heading", { name: "Quote List" }).waitFor();
    await page.locator("#dashboardLoadingModal").waitFor({ state: "hidden", timeout: 15000 });
