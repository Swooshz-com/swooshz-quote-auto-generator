...
try {
  ...
  const exportEvidence = await new Promise((resolve, reject) => {
    const child = spawn(process.execPath, ["scripts/playwright-smoke.mjs", "--run552-authenticated-only"], {
      cwd: process.cwd(),
      env: { ...process.env, RUN552_AUTHENTICATED_BASE_URL: baseUrl },
      stdio: ["ignore", "pipe", "pipe"],
    });
    let stdout = "";
    let childStderr = "";
  ...
  if (!exportEvidence.includes("Run-552 native authenticated export persistence: PASS")) {
